		...
		methods: {
			...
			populateData: function() {
				...
				for(i=0; i<this.transactions.length; i++) {
					...
					var combine_desc = (item.formatteddate+','+item.description+','+item.category+','+item.account+','+item.amount).toLowerCase();
			...
			saveTransaction: function() {
				...
				if($('#amount').val()=='0' || $('#amount').val()=='') $('#description').val('Reconcile');
