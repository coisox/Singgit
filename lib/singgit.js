		...
		methods: {
			...
			populateData: function() {
				...
				for(i=0; i<this.transactions.length; i++) {
					...
					if((item.description=='Reconcile' && !firstReconcile) || (item.description + item.account[0] + item.account[1] + item.amount).toLowerCase().indexOf(this.filter.toLowerCase())>-1) {
						if(item.description=='Reconcile' && !firstReconcile) firstReconcile = true;
						data.push(
							'<li class="list-group-item d-flex align-items-center" onclick="singgit.editTransaction('+data.length+')">'+
