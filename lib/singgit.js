		...
		methods: {
			...
			saveTransaction: function() {
				...
				if($('#amount').val()=='0' || $('#description').val()=='Reconcile') {
					...
					if($('#description').val()=='') $('#description').val('Transfer');
			...
			filter: function(item) {
				var pass = true;
				var fulltext = (item.description+', '+item.account[0]+', '+item.account[1]+', '+item.category).toLowerCase();
				
				if(singgit.search.indexOf(' and ')>-1) {
					var arrSearch = singgit.search.split(' and ');
					for(x=0; x<arrSearch.length; x++) if(fulltext.indexOf(arrSearch[x])<0) pass = false;
				}
				else if(singgit.search.indexOf(' or ')) {
					var arrSearch = singgit.search.split(' or ');
					var pass2 = false;
					for(x=0; x<arrSearch.length; x++) if(fulltext.indexOf(arrSearch[x])>-1) pass2 = true;
					pass = pass2;
				}
				else {
					if(fulltext.indexOf(singgit.search)<0) pass = false;
				}
