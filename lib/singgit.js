...
firebase.firestore().enablePersistence().then(function() {
	...
	singgit = new Vue({
		...
		methods: {
			...
			deleteAutocomplete: function(index) {
				this.autocomplete.splice(index, 1);
			},
			draftAutocomplete: function() {
				var _this = this;
				_this.autocompleteLoading = true;
				
				db.doc("autocomplete").get().then(function(querySnapshot) {
					_this.autocomplete = _this.autocomplete.concat(querySnapshot.data()['data']);
					for(var i=0; i<_this.autocomplete.length; i++) _this.autocomplete[i] = JSON.stringify(_this.autocomplete[i]);
					_this.autocomplete = [... new Set(_this.autocomplete)];
					for(var i=0; i<_this.autocomplete.length; i++) _this.autocomplete[i] = JSON.parse(_this.autocomplete[i]);
					
					_this.autocomplete.sort(function(a, b){
			...
			saveAutocomplete: function() {
				...
				//====================================================================================================== save autocomplete to firestore
				db.doc('autocomplete').set({data:_this.autocomplete}).then(function(){
					$('#modalGeneral .modal-body').text('Done');
					$('#modalGeneral').modal('show');
				});
			},
		...
		mounted: function() {
			...
			//============================================================================================================== fetch autocomplete
			_this.autocomplete = JSON.parse(localStorage.getItem('SINGGIT_Autocomplete')?localStorage.getItem('SINGGIT_Autocomplete'):'[]');
