	...
	singgit = new Vue({
		...
		methods: {
			...
			editTransaction: function(id) {
				...
				for(x=0; x<singgit.transactions.length; x++) {
					if(singgit.transactions[x].id==id) {
						$('#amount').val(singgit.transactions[x].amount*-1);
				...
				previous = {};
				previous.amount = $('#amount').val()*-1;
				previous.account = $('#account').val();
				previous.transfer = $('#transfer').val();
			},
			...
			saveTransaction: function() {
				...
				for(r=0; r<$('#repeat').val(); r++) {
					if(singgit.activeTab=='edit' && r==0) {
						...
						singgit.balance[$('#account').val()] -= Number(eval($('#amount').val()));
						singgit.balance[$('#account').val()] = Number(singgit.balance[$('#account').val()].toFixed(2));
						if($('#transfer').val()!='') {
							singgit.balance[$('#transfer').val()] += Number(eval($('#amount').val()));
